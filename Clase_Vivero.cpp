...
class Arbol:public Hoja, public Tallo{
	private:
		string nomComun;//nombre comun
		string nomCient;//nombre cientifico del arbol
		string familia;//familia a la que pertenece
		Hoja *formHoja;
		Hoja *tamHoja;
		Tallo *altTallo;
		Tallo *gTallo;
	...
	do{
		...
		switch(opc){
			...
			case 1:
				...
				cout<<"Desesa agregar otro arbol? y/n"<<endl;
				cin>>resp;
				system ( "cls" ) ;      //limpiar
			}while (resp == 'y' || resp == 'Y'); 
				system("pause");
